...
class Car():
    def __init__(self, carInfLine_):
        ...
        self.delay = 0
        self.goTime, self.stopTime = None, None
...
def main():
    ...
    relate_path = 'Map/2-training-training-1-answer'
    ...
    while CARDISTRIBUTION[0] + CARDISTRIBUTION[1] != 0:
        ...
        print(TIME[0], CARDISTRIBUTION) #3580 1.43165569 7.9064910632 1.45317 8.22387
...
if __name__ == "__main__":
    ...
    while not finish_sign:
        ...
        ROADLOSSDICT, ROADUSEDICT = {}, {} #{crossId: {crossId: loss}}
        finish_sign = main()
    allCarTime, allCarTotalTime, priCarTime, priCarTotalTime = 0, 0, 0, 0
    priCarFirstTime, allCarFirstTime, priCarEndTime, allCarEndTime = 10000, 100000, 0, 0
    for carId in CARNAMESPACE:
        car = CARDICT[carId]
        allCarTotalTime += car.stopTime - car.startTime_
        allCarFirstTime = min(allCarFirstTime, car.startTime_)
        allCarEndTime = max(allCarEndTime, car.stopTime)
        if car.priority_:
            priCarTotalTime += car.stopTime - car.startTime_
            priCarFirstTime = min(priCarFirstTime, car.startTime_)
            priCarEndTime = max(priCarEndTime, car.stopTime)
    priCarTime = priCarEndTime - priCarFirstTime
    allCarTime = allCarEndTime
    print(time.time() - a)
    print(allCarTime, allCarTotalTime, priCarTime, priCarTotalTime)
