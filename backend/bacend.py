...
def update_loss(array_loss, array_dis, road_inf, road_percent_list, road_id_bias, speed, cross_loss):
    ...
    for i in road_inf:
        ...
        # loss = length * 1 / min(speed_lim, speed) +  * cross_loss[start_id][end_id]
        loss = length + 500 * cross_loss[start_id][end_id] + 10000 * use_rate / channel
        # 100000, 1000 ; 
        if is_dux == 1:
    ...
    for i in range(a):
        for j in range(a):
            if map_road_array[i][j] not in [10000, 0]:
                ...
                loss = max((in_flow / num_avail / road_flow[cross[dir_out]] - 1), 0) / road[3]
