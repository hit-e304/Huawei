...
def printTxt(list, save_path):
    with open(save_path, 'w') as fp:
        fp.write('\n'.join(str(tuple(x)) for x in list))
...
def main():
    ...
    for speed in speed_list:
        ...
        for batch in group_divide_time:
            ...
            printTxt(answer, 'answer.txt')
            printTxt(car_inf, 'car.txt')
            printTxt(road_inf, 'road.txt')
            printTxt(cross_inf, 'cross.txt')
            # changeAnswer(batch_path_time)
