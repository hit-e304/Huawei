...
class CROSS(object):
    ...
    def step(self):
        ...
        for provideIndex in range(self.provider.__len__()): # provideIndex [0, 1, 2, 3]
            nextCarId.append(ROADDICT[self.provider[provideIndex]].firstPriorityCar()) # [fisrt car in 5001, fisrt car in 5002 ....]
            # if first priority car exists
            if nextCarId[provideIndex]!=-1:
                nextCar.append(CARDICT[nextCarId[provideIndex]]) # [Car..., Car..., Car..., ]
                nextRoad.append(nextCar[provideIndex].__nextRoad__()) # [5007, 5007, 5008, 5001]
                # nextRoad == -1 => terminal
