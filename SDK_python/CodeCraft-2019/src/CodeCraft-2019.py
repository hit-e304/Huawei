...
# def calc_heuristic(n1, n2, cross_map:
...
def update_loss(array_loss, array_dis, road_inf, road_use_dic, road_id_bias, speed, cross_loss):
    ...
    for i in road_inf:
        ...
        array_loss[start_id][end_id] = length * 1 / min(speed_lim, speed) + delta1 * forward_use + delta2 * cross_loss[start_id][end_id]

        if is_dux == 1:
            # array_dis[end_id][start_id] = length * (1 / min(speed_lim, speed) + 100 * backward_use) + 200 * cross_loss[start_id][end_id]
            array_loss[end_id][start_id] = length * 1 / min(speed_lim, speed) + delta1 * backward_use + delta2 * cross_loss[end_id][start_id]
        else:
...
def printCrossLoss(road_inf, cross_loss, loss_path):
    '''
...
def main():
    ...
    for speed in speed_list:
        ...
        printCrossLoss(road_inf, cross_loss, loss_path)
