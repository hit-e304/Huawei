...
def update_loss(array_loss, array_dis, road_inf, road_percent_list, road_id_bias, speed, cross_loss):
    for i in road_inf:
        ...
        loss = length * (1 / min(speed_lim, speed) + 30 * use_rate / channel) + 0.1 * max(cross_loss[start_id + 1],
                                                                                          cross_loss[end_id + 1])
...
def main():
    ...
    for speed in speed_list:
        ...
        sort_group = car_sort_time(cur_group, speed, car_inf, road_inf, map_dis_array, map_road_array, car_id_bias,
                                   road_id_bias)
        group_divide_time = time_split(sort_group, car_inf, car_per_sec, time, interval_time, speed, car_id_bias,
                                       car_position)
